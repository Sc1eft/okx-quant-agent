...
def _paper_monitor_fragment():
    ...
    # ── 4. PROCESSING CORE — real-time loop（由 fragment 定时驱动，无需 sleep+rerun）──
    if running and strategy is not None:
        ...
        if df is not None and not df.empty:
            ...
            if processed < total:
                # ── Phase 2: replay cached bars（单批次回放，替代逐根 sleep+rerun 全页刷新）──
                with st.spinner(f"⏳ 回放历史 K 线 ({processed}/{total})..."):
                ...
                st.toast(f"✅ 历史回放完成，共 {total} 根 K 线", icon="📊")

            else:
                # ── Phase 3: real-time polling（由 fragment 定时触发，无需 sleep+rerun）──
